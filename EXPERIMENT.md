...
## 5. GS → 삼각형 변환 (C) — 선택적 · rank-aware 승격

분리형에선 삼각형이 B에서 A와 동일하므로, **삼각형이 실제로 바뀌는 유일한 단계가 C다.**

1. B 체크포인트(`triangles/` + `gaussians.pt`) 로드
2. **승격 필터** (모든 GS를 변환하지 않음):
   - (a) confident: opacity ≥ `opacity_threshold`
   - (b) surface-like(rank≈2): `s2/s1 ≥ surface_ratio` AND `s3/s1 ≤ flat_ratio`
   - 둘 다 만족한 GS만 승격. **needle(rank1)·blob(rank3, fuzzy)은 제외** — 평면 quad가
     안 맞고, fuzzy appearance residual은 mesh로 만들면 안 되므로. (승격 안 된 영역은
     baseline 삼각형으로 남는다.)
3. 승격된 GS를 **삼각형 패치로 변환**(GS 1개 → 삼각형 2개, `size_factor`) 후 원본
   `TriangleModel`에 주입 → **삼각형만으로 finetune**.
4. **주입 patch 보호**: finetune 초반 `protect_iters` iter 동안 pruning/densify 정지 +
   매 iter importance/size bookkeeping 갱신 → 방금 주입한 patch가 첫 maintenance에서
   잘려나가지 않고 importance를 쌓을 시간을 준다.
5. 최종 평가는 **GS 없이 삼각형 렌더만**, held-out test셋 평균(A와 like-for-like).
   `summary.json`의 `promotion`(n_total/surface_like/confident/promoted)과
   `triangles_before/after_conversion`으로 승격·생존을 확인한다.
...
- **C 승격 임계값은 튜닝 대상** — `opacity_threshold`(0.2)·`surface_ratio`(0.3)·`flat_ratio`(0.3)는
  씬의 opacity/scale 분포에 따라 조정 필요. `summary.json`의 `promotion` 통계로 확인.
- **C finetune은 `protect_iters` 이후 densify도 함** (iteration 카운터가 upstream 스케줄에
  걸림). 필요하면 별도 정지 고려.
